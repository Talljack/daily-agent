...
export async function buildDailyReport(): Promise<DailyReport> {
  ...
  if (remoteResults.length > 0) {
    const formatted = buildRemoteJobsEmailContent(remoteResults, report.generatedAt);
    report.emailSubject = formatted.subject;
    report.emailMarkdown = formatted.markdown;
  }
